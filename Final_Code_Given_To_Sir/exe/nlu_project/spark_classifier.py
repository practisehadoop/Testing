...
class classifier:
            ...
            if (status['status'] == 'Available' and len(df2.review_text) > 0 ):
                        for i in range(0,len(df2.review_text),1):
                               ...
                               # print line#ISO-8859-1
                                ...
                                classes.append(natural_language_classifier.classify('359f41x201-nlc-65743',line))
                        with open(spark_conf.file_path['data_update_path']+'review_{}.json'.format('classifier'),'w') as f:
    ...
    def confident_classifier(self,cc_input):
       # try:
            logging.info('Adding Confidence and class to the file')
            df3 = pd.read_json(spark_conf.file_path['data_update_path']+'review_{}.json'.format('classifier'))
            #print df3
    ...
    def linear_regression(self,lr_input,sqlContext,cc_output):
       ...
       # try:
           ...
           # dfi_test['xyz'] =  se.values
            ...
            #df_s.show()
            #print df_s.count()
            df_s.rdd.map(lambda x: list(x)).map(lambda y: filter_data(y)).saveAsTextFile(spark_conf.hdfs_path['classifier_output']+ '%s.txt' %spark_conf.utc_time()[1])
